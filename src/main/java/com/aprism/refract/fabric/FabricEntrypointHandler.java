...
/**
 * The Fabric entrypoint-dispatch handler. Registered against loader key
 * {@code Fa} via {@code ExtensionContext.registerEntrypointHandler}, it fully
 * owns Fabric (and Quilt's Fabric-compatibility) entrypoint dispatch — the
 * Aprism core no longer needs any Fabric-specific translation code.
 *
 ...
 * handler supplies the Fabric-specific behaviour, and the Fabric API shims
 * ({@code net.fabricmc.api.*}) are bundled in the same extension jar.
 *
 ...
 *       manifest (Fabric's {@code fabric.mod.json} is projected to an Aprism
 *       manifest; the Quilt-native {@code init} key is projected to
 *       {@code main}).</li>
 ...
 *   <li>Otherwise invoke the Fabric-convention entrypoint
 ...
 */
public final class FabricEntrypointHandler implements LoaderEntrypointHandler {

    /** The Fabric loader key reserved in Aprism FACT.md 9.14. */
    public static final String FABRIC_KEY = "Fa";

    /** The Quilt loader key; Quilt mods are dispatched through the Fabric
     *  compatibility convention, so this handler serves both keys. */
    public static final String QUILT_KEY = "Q";

    private static final Logger LOG = Logger.getLogger(FabricEntrypointHandler.class.getName());
     ...
     */
    public FabricEntrypointHandler(AprismEventBus eventBus, AprismRegistry registry,
    ...
    /**
     * Fully owns Fabric/Quilt dispatch: the Aprism-native
     * {@code IAprismMod} fallback must NOT run afterwards, exactly like the
     * former core built-in path (which returned after dispatching).
     */
    @Override
    ...
    public void invoke(LoadedModContainer container, AprismPhase phase) {
        ...
        FabricBridge.registerMod(container.getId());
        ...
        for (String className : entrypoints) {
            try {
                ...
                if (instance instanceof IAprismMod mod) {
                    // Aprism-native mod living under fabric-mods/: full lifecycle
                    invokePhaseMethod(mod, container, phase);
                } else {
                    // Fabric-convention entrypoint
    ...
    /**
     * Maps a lifecycle phase to the Fabric entrypoint key, matching the core
     * convention ({@code main} for PREINIT/INIT/SETUP/COMPLETE, {@code client}
     * for CLIENT, {@code server} for SERVER).
     *
     * @param phase the lifecycle phase
